...
- You can run generation/build commands:
  - `pnpm --filter @veltodefi/ai-docs generate`
...
3. **`samples` (optional)**: comma-separated representative symbols
   - Example: `useMarkPrice,useOrderEntry,@veltodefi/ui:Avatar`
...
```bash
@ai-docs-source-metadata-enrichment scope=components samples=@veltodefi/ui:Avatar,@veltodefi/ui:Button
```
