...
- **Prefix**: All style-override classNames start with `oui-`. Map package to module name (e.g. `@veltodefi/trading` → `oui-trading`, `@veltodefi/markets` → `oui-markets`). For other packages, use the last segment or main module name (e.g. `@veltodefi/xyz` → `oui-xyz`); if the package name differs from the page, use the team’s agreed module name.
