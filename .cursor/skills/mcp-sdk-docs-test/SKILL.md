...
description: Execute MCP Inspector testing workflow for @veltodefi/sdk-docs. Use when user asks to test MCP tools, validate stdio inspector setup, run smoke/regression checks, or troubleshoot MCP JSON parse issues.
---

# MCP SDK Docs Test Skill

Use this skill to run a stable MCP test workflow for `@veltodefi/sdk-docs`, especially with MCP Inspector (`stdio`).
...
```bash
pnpm --filter @veltodefi/sdk-docs build
```
...
- `orderly_docs_get_package_surface` with `{ "packageName": "@veltodefi/ui" }`
