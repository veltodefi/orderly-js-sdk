...
- Use **one** root **`tsconfig.json`** that **`extends`**: `"@veltodefi/ts-config/react-library.json"`.
- Add to **devDependencies**: `"@veltodefi/ts-config": "<version>"`. Use a version from the monorepo root, other packages, or published npm.
- **Remove** from **devDependencies** the monorepo workspace **`tsconfig`** entry (e.g. `"tsconfig": "workspace:*"`). It is replaced by `@veltodefi/ts-config`.
...
```json
{
  "extends": "@veltodefi/ts-config/react-library.json",
...
- **If** the package has a `tailwind.config.cjs` that references a local UI preset, replace that preset with the package reference:
  ...
  - To: `presets: [require("@veltodefi/ui/tailwind.config.js")]`
...
module.exports = {
  ...
  presets: [require("@veltodefi/ui/tailwind.config.js")],
};
```
...
If **`pnpm run build:css`** (or equivalent) fails because a **plugin** required by `@veltodefi/ui`’s Tailwind preset is not installed in the standalone repo (e.g. missing module `tailwindcss-animate`), add that package to **devDependencies** so the CLI can resolve it.
...
- [ ] devDependencies includes `@veltodefi/ts-config` and no longer lists workspace **`tsconfig`**.
- [ ] Single root `tsconfig.json` extends `@veltodefi/ts-config/react-library.json`, has no monorepo `paths`; `typeRoots` is removed unless custom `./src/@types` (or equivalent) still requires it.
...
- [ ] `tailwind.config.cjs` uses `presets: [require("@veltodefi/ui/tailwind.config.js")]`.
