...
- Use workspace imports: `import { X } from "@veltodefi/package-name"`
...
- Use `Decimal` from `@veltodefi/utils` for numeric calculations
- Use `useLocalStorage` from `@veltodefi/hooks` for local caching
...
1. Search `@veltodefi/i18n` for existing keys first
...
Always use `Decimal` from `@veltodefi/utils`:
...
Use `useLocalStorage` hook from `@veltodefi/hooks`:
...
- Use `@veltodefi/ui` components when available
...
| Package                    | Purpose                   |
| -------------------------- | ------------------------- |
| `@veltodefi/ui`      | Base UI components        |
| `@veltodefi/hooks`   | React hooks               |
| `@veltodefi/utils`   | Utilities (Decimal, etc.) |
| `@veltodefi/i18n`    | Internationalization      |
| `@veltodefi/types`   | TypeScript types          |
| `@veltodefi/markets` | Market data               |
| `@veltodefi/perp`    | Perpetual trading logic   |
