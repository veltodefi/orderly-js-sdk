...
Provides generic account-based filtering for list data and a dedicated filter for transfer history (by `from_account_id` / `to_account_id`). Uses `useAccount` from `@veltodefi/hooks` and `AccountType` from assets UI.
