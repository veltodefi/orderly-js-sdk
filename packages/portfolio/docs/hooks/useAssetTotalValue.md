...
- **Upstream**: useAccount, useCollateral, useIndexPricesStream from `@veltodefi/hooks`; useAccountsData from same package; Decimal from `@veltodefi/utils`.
...
const total = calculateTotalHolding(
  subAccounts,
  (token) => indexPrices[token] ?? 0,
);
```
