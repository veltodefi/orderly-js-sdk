...
`EthersProvider` is a TypeScript class that implements the `Web3Provider` interface from `@veltodefi/default-evm-adapter`. It wraps `ethers` v6 `BrowserProvider` (and an underlying EIP-1193 provider) to provide signing, contract calls, transaction sending, and balance queries used by Orderly's EVM integration.
