...
- **Base SDK strings:** glob `node_modules/@veltodefi/i18n/dist/locales/*.json` (path may differ slightly depending on your install layout).
...
const baseLoaders = import.meta.glob<LocaleJsonModule>(
  "/node_modules/@veltodefi/i18n/dist/locales/*.json",
);
...
async function loadBase(lang: LocaleCode): Promise<Record<string, string>> {
  const key = `/node_modules/@veltodefi/i18n/dist/locales/${lang}.json`;
...
- **Base SDK strings:** import from **`@veltodefi/i18n/locales/<locale>.json`** (published subpaths; add an entry for **every** `LocaleEnum` value you support).
...
import type { LocaleJsonModule } from "@veltodefi/i18n";
import { OrderlyAppProvider } from "@veltodefi/react-app";
import { WalletConnectorProvider } from "@veltodefi/wallet-connector";
import extendEnLocale from "@/locales/en.json";
...
/**
 * One explicit import per locale so webpack can resolve @veltodefi/i18n/locales/*.json
...
const baseLocaleLoaders: Record<LocaleEnum, LocaleJsonLoader> = {
  [LocaleEnum.de]: () => import("@veltodefi/i18n/locales/de.json"),
  [LocaleEnum.en]: () => import("@veltodefi/i18n/locales/en.json"),
  [LocaleEnum.es]: () => import("@veltodefi/i18n/locales/es.json"),
  [LocaleEnum.fr]: () => import("@veltodefi/i18n/locales/fr.json"),
  [LocaleEnum.id]: () => import("@veltodefi/i18n/locales/id.json"),
  [LocaleEnum.it]: () => import("@veltodefi/i18n/locales/it.json"),
  [LocaleEnum.ja]: () => import("@veltodefi/i18n/locales/ja.json"),
  [LocaleEnum.ko]: () => import("@veltodefi/i18n/locales/ko.json"),
  [LocaleEnum.nl]: () => import("@veltodefi/i18n/locales/nl.json"),
  [LocaleEnum.pl]: () => import("@veltodefi/i18n/locales/pl.json"),
  [LocaleEnum.pt]: () => import("@veltodefi/i18n/locales/pt.json"),
  [LocaleEnum.ru]: () => import("@veltodefi/i18n/locales/ru.json"),
  [LocaleEnum.tc]: () => import("@veltodefi/i18n/locales/tc.json"),
  [LocaleEnum.tr]: () => import("@veltodefi/i18n/locales/tr.json"),
  [LocaleEnum.uk]: () => import("@veltodefi/i18n/locales/uk.json"),
  [LocaleEnum.vi]: () => import("@veltodefi/i18n/locales/vi.json"),
  [LocaleEnum.zh]: () => import("@veltodefi/i18n/locales/zh.json"),
};
...
`backend.loadPath` only returns URL(s). The internal `Backend` **`fetch`es** those URLs at runtime. **`@veltodefi/i18n` does not install or copy** `dist/locales` into your app’s `public/` folder — you must **provide files** at those paths yourself.

### Getting built-in JSON into `public/`

**Manual:** Copy from `node_modules/@veltodefi/i18n/dist/locales` (exact path depends on your install layout) into e.g. **`public/locales/`**. Put your app **extend** JSON under e.g. **`public/locales/extend/`** (same layout as `loadPath` below).
...
async function copyLocales() {
  await fs.copy(
    path.resolve(__dirname, "../node_modules/@veltodefi/i18n/dist/locales"),
...
Use this when you bundle locale JSON (e.g. import from `@veltodefi/i18n/locales/*`) and do not need async loading.

```typescript
import { FC, ReactNode } from "react";
import { WalletConnectorProvider } from "@veltodefi/wallet-connector";
import { OrderlyAppProvider } from "@veltodefi/react-app";
import { LocaleProvider, Resources } from "@veltodefi/i18n";
...
Import **`LocaleCode`** and **`removeLangPrefix`** from `@veltodefi/i18n` and pass the callback to **`LocaleProvider`** as **`onLanguageChanged`**. Strip the current locale segment from the pathname, then use **`history.replaceState`** to set the path to `/${lang}${path}`.
...
import { WalletConnectorProvider } from "@veltodefi/wallet-connector";
import { OrderlyAppProvider } from "@veltodefi/react-app";
import { LocaleProvider, LocaleEnum, LocaleCode, removeLangPrefix } from "@veltodefi/i18n";
...
const OrderlyProvider: FC<{ children: ReactNode }> = (props) => {
  ...
  // Please copy built-in locale files (@veltodefi/i18n/locales) to your public/locales
