...
The `i18n` binary ships with `@veltodefi/i18n`. It helps maintain locale files: CSV/JSON conversion, CSV diffs, merging default and `extend/` JSON, splitting by key prefix, and filtering keys. The binary name is **`i18n`** (see `package.json` `bin`).
...
- `npx @veltodefi/i18n` — runs the published package without a prior install

Below, examples use `npx @veltodefi/i18n`; substitute `pnpm exec i18n` or `npx i18n` as needed.

**Help:** run `npx @veltodefi/i18n --help` for the command list, or `npx @veltodefi/i18n <command> --help` for a subcommand (e.g. positionals and flags for `csv2json`).
...
**Example:** `npx @veltodefi/i18n csv2json ./dist/locale.csv ./dist/locales`
...
**Example:** `npx @veltodefi/i18n json2csv ./locales ./dist/locale.csv`
...
**Example:** `npx @veltodefi/i18n diffcsv ./dist/locale1.csv ./dist/locale2.csv`
...
```bash
npx @veltodefi/i18n generateCsv <inputDir> <output>
```

**Example:** `npx @veltodefi/i18n generateCsv ./locales ./dist/locale.csv`
...
**Example:** `npx @veltodefi/i18n fillJson ./src/locale/zh.json ./dist/locale/zh.json`
...
```bash
npx @veltodefi/i18n separateJson <inputDir> <outputDir> <keyPrefix>
```

**Example:** `npx @veltodefi/i18n separateJson ./locales ./dist/locales extend`
...
**Example:** `npx @veltodefi/i18n mergeJson ./locales ./dist/locales`
...
```bash
npx @veltodefi/i18n filterKeys (--keep | -k | --remove | -r) --prefix <prefix> [--locales-dir <dir>]
```
...
```bash
npx @veltodefi/i18n filterKeys --keep --prefix trading.
npx @veltodefi/i18n filterKeys -r --prefix trading.
npx @veltodefi/i18n filterKeys -k --prefix extend. --locales-dir ./my-locales
```
