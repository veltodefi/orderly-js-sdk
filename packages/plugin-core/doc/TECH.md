...
- **Widget**: Uses **path-based interceptors** and injected `api` (or `@veltodefi/hooks` directly); main way to extend trading UI.
...
- **Interceptor shape**: `component` is `(Original, props, api) => ReactNode`—wrap, prepend, append, or replace `Original`; you may also use `@veltodefi/hooks` directly.
...
Use `@veltodefi/hooks` inside a wrapper component—not in the outer interceptor function (Rules of Hooks).
...
Plugins may **import `@veltodefi/hooks` directly** or use an injected **`api`** (from `OrderlyPluginProvider`) for `setup` and similar non-component contexts.

The concrete `api` shape in your SDK version may include `events` and placeholders for future `data` / `actions` / `utils`—see `@veltodefi/plugin-core` and [GUIDE.md](./GUIDE.md) §3.
