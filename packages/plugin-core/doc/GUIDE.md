...
- **Integration**: Intercept **`Trading.Layout.Desktop`**. When the host does not pass `layoutStrategy` / `getInitialLayout`, the plugin overrides props on `DesktopLayout`, injecting strategy and `getInitialLayout`, which still render `LayoutHost`. Strategy types come from `@veltodefi/layout-core`.
...
- **Example**: `@veltodefi/layout-grid` exposes **`registerLayoutGridPlugin(options?)`**, intercepts `Trading.Layout.Desktop`, and injects the grid strategy when the host omits it; use `plugins={[registerLayoutGridPlugin()]}` or supply `getInitialLayout` for a custom initial grid.
...
```typescript
// @veltodefi/plugin-example/index.ts
import React from "react";
import type { OrderlySDK } from "@veltodefi/ui";
import { useOrderEntry } from "@veltodefi/hooks";
...
```typescript
import { createInterceptor } from "@veltodefi/plugin-core";
import "@veltodefi/ui"; // pull in augmentation
...
```typescript
import type { PluginInterceptor, DepositFormProps } from '@veltodefi/ui';
...
```typescript
import type { DepositFormProps } from '@veltodefi/ui';

component: (Original, props: DepositFormProps, api) => { ... }
```

UI packages (`@veltodefi/ui`, `@veltodefi/ui-scaffold`, `@veltodefi/trading`) export Props types for their injectable components—import as needed.
...
- **What developers see**: Set `id` in `registerPlugin({ id: '...' })`. When interceptor output renders, `PluginScopeProvider` wraps it; `@veltodefi/hooks` hooks such as `useQuery` read `pluginId` via `usePluginScope()` and attach the `X-Orderly-Plugin-Id` header.
...
- **`@veltodefi/hooks`**: Import and use the same hooks as the host (`useAccount`, `useOrderBook`, …). **Prefer using hooks inside wrapper components returned from interceptors.**
...
Supported events include `place_order_success`, `deposit_success`, `withdraw_success`, etc. (`TrackerEventName` in `@veltodefi/types`).
...
`component` is `(Original, props, api) => ReactNode`: `Original` is the inner component, `props` are the anchor props, `api` is the plugin API. Wrap, prepend, append, or replace `Original` in JSX. **Again:** the outer function is not a component—return an inner component if you need hooks; data and actions come from `@veltodefi/hooks`.
...
> All of the above support typed `props` via `createInterceptor(target, (Original, props) => ...)` after importing `@veltodefi/ui-order-entry`.
...
import React from "react";
import { createInterceptor } from "@veltodefi/plugin-core";
import "@veltodefi/ui-order-entry";
...
### 5.2 Prefer `@veltodefi/ui`

When building plugin UI, **prefer** components from `@veltodefi/ui`:

- **Layout**: `Box`, `Flex`, `Grid` (and `Grid.span`)
- **Interaction**: `Button`, `Input`, `Checkbox`, `Dialog`, `Sheet`, `Tabs`, etc.

See the **`@veltodefi/ui`** package for the full list. Avoid bare `<button>`, `<input>`, or unstyled `<div>` stacks when a component exists—they bypass tokens and theming.
...
```tsx
import { Flex, Button } from "@veltodefi/ui";
...
More layout props are defined in **`@veltodefi/ui`** exports and types.

### 5.4 Custom `className` and the `oui-` prefix

Host and UI packages typically enable Tailwind with the **`oui-` prefix** (`OUITailwind` preset from **`@veltodefi/ui`**). When adding styles in plugins:
...
If the plugin package maintains its own `tailwind.config`, import Orderly’s preset and plugins (`import { OUITailwind } from "@veltodefi/ui"`, `presets: [OUITailwind.theme]`, `OUITailwind.base()`, `OUITailwind.components()`, etc.) so `oui-*` output matches the host.

### 5.5 Common CSS variables (`--oui-*`)

A short reference for custom CSS; the **full set** lives in the installed **`@veltodefi/ui`** light/dark themes (names are stable across themes, values change).
...
Color variables use **RGB triplets (space-separated, no commas)** for `rgb(var(--oui-color-primary) / <alpha-value>)`, not `#RRGGBB` strings. Prefer `oui-*` Tailwind classes; if you write raw CSS, align with your **`@veltodefi/ui`** version.
...
| **Layout** | Trading page layout only. Intercept **`Trading.Layout.Desktop`** and inject `layoutStrategy` / `getInitialLayout` when the host omits them; strategies from `@veltodefi/layout-core`. | Multi-column, portrait mobile, pro/minimal modes. |
...
Install the plugin via npm and call its register function during bootstrap; pass the **invoked** factories to `OrderlyPluginProvider` (`@veltodefi/ui`) `plugins`—e.g. `registerPnlPlugin()` or `registerCustomThemePlugin({ primaryColor: '#00ff00' })`. The SDK passes the current instance and optional `pluginState`; hosts do not hand in `SDK` manually:

```tsx
// App.tsx
import registerPnlPlugin from "@veltodefi/plugin-pnl";
