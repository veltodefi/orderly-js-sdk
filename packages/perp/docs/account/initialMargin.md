...
- **Upstream**: `@veltodefi/types` (API, OrderSide, MarginMode), `@veltodefi/utils` (Decimal, zero), `../constants` (IMRFactorPower), positionNotional (positionQtyWithOrders_by_symbol, positionNotionalWithOrder_by_symbol), positionUtils (getQtyFromPositions, getQtyFromOrdersBySide).

## Example

```typescript
import { totalInitialMarginWithQty } from "@veltodefi/perp";
