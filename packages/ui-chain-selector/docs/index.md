...
Overview of the `@veltodefi/ui-chain-selector` package source. This module provides chain selector UI components, types, and scripts for switching networks (mainnet/testnet) and registering dialog/sheet presets.
...
Usage: open the selector via `openSimpleDialog(ChainSelectorDialogId, ...)` or `openSimpleSheet(ChainSelectorSheetId, ...)` from `@veltodefi/ui`.
