...
Implements the chain selector logic: hook `useChainSelectorScript` and internal helpers for chain data, recent chains, and tab state. Uses `@veltodefi/hooks` (config, storage chain, chains, wallet connector), `@veltodefi/types` (NetworkId), and `@veltodefi/react-app` (onChainChanged, currentChainId, wrongNetwork).
