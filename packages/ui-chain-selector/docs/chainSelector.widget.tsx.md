...
Composes the chain selector script and UI into a single widget and registers it as a simple dialog and a simple sheet in `@veltodefi/ui`. Exports the widget, its props type, and the dialog/sheet IDs used to open it.
...
import { openSimpleDialog, openSimpleSheet } from "@veltodefi/ui";
