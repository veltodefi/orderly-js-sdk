...
SVG icon components used in notification and announcement headers. All accept `BaseIconProps` from `@veltodefi/ui` (e.g. `size`, `color`) and use the shared `Icon` wrapper with `viewBox="0 0 18 18"`.
...
All components extend `BaseIconProps` from `@veltodefi/ui` (e.g. `size`, `color`, `className`). Default `size` is 18.
