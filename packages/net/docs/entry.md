...
包入口文件，聚合并 re-export 版本、HTTP 方法、常量、WebSocket 类与事件枚举，供上层通过 `@veltodefi/net` 单一入口使用。
...
| **ORDERLY_API_URL_KEY**     | string constant         | 常量               | 来自 `./constants` |
| WS                          | class                   | WebSocket 客户端   | 来自 `./ws/ws`     |
| WebSocketEvent              | enum                    | WebSocket 事件枚举 | 来自 `./ws/ws`     |

## index.ts 依赖与关联关系

- 上游调用方：使用 `@veltodefi/net` 的应用或包
