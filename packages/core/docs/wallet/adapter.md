...
- **Upstream**: ethers (TransactionResponse), @veltodefi/types (API).
- **Downstream**: etherAdapter.ts, web3Adapter.ts implement; wallet/index.ts re-exports as WalletAdapter type.

## adapter.ts Example

```typescript
import type { IWalletAdapter, WalletAdapterOptions } from "@veltodefi/core";

const options: WalletAdapterOptions = {
  provider,
  address: "0x...",
  chain: { id: 421614 },
};
