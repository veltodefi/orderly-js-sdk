...
Re-exports wallet adapter types from `./adapter`: `IWalletAdapter` as `WalletAdapter`, `getWalletAdapterFunc`, `WalletAdapterOptions`. Package consumers importing from `@veltodefi/core` wallet get these from the main index re-export of `./wallet`.
