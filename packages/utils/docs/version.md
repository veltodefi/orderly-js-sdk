...
When `window` is defined, it sets `window.__ORDERLY_VERSION__["@veltodefi/utils"]` to the package version.

## Usage example

```typescript
import version from "@veltodefi/utils";

console.log(version); // "2.9.1"

// In browser: window.__ORDERLY_VERSION__?.["@veltodefi/utils"] === "2.9.1"
```
