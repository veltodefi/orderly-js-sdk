...
When running in a browser, sets `window.__ORDERLY_VERSION__["@veltodefi/wallet-connector"] = "2.9.1"` (or merges into existing object).

## Usage example

```ts
import version from "@veltodefi/wallet-connector/version";

console.log(version); // "2.9.1"

// In browser:
// window.__ORDERLY_VERSION__?.["@veltodefi/wallet-connector"] === "2.9.1"
```
