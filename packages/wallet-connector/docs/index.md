...
| [chains](chains.md) | TypeScript | `NetworkInterface`, `getChainsArray()` from `@veltodefi/types`. |
...
Wrap your app with `WalletConnectorProvider` and pass optional `solanaInitial` and `evmInitial` props. Consume the connector state via `@veltodefi/hooks` (e.g. `WalletConnectorContext`).
