...
Root React provider for the wallet connector. Composes `SolanaProvider`, `InitEvm`, and `Main` so a single provider can drive both Solana and EVM wallet state and expose it via `WalletConnectorContext` from `@veltodefi/hooks`.
