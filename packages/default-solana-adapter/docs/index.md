...
This package provides the **default Solana wallet adapter** for Orderly Network. It implements `BaseWalletAdapter` from `@veltodefi/core` for the Solana chain namespace, handling account registration, key management, message signing (including Ledger), deposits via LayerZero vault, and integration with the Solana vault IDL.
