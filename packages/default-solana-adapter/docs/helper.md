...
Provides message-building helpers for Orderly operations (add orderly key, register account, internal transfer, withdraw, settle, DEX request) and vault deposit/quote fee logic. Uses `@veltodefi/core` input types, ethers/solidity hashing, and Anchor Program for the Solana vault IDL.
