...
| ---------------------------- | -------- | ----------- | ------------------------------------------------------------------------------- |
| RefereeDataType              | type     | Row shape   | account*id, address, bind_code, volume, commission, direct*_, indirect\__, etc. |
...
- react (useMemo)
- @veltodefi/hooks (useAccount, usePrivateQuery)
- @veltodefi/types (AccountStatusEnum)

## hooks/useMultiLevelReferees.ts Example

```typescript
const { rows, meta, isLoading } = useMultiLevelReferees({
  page: 1,
  pageSize: 10,
});
```
