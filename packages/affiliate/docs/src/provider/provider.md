...
`ReferralProvider` wraps the app (or affiliate section) and provides referral state: referral info from API, generate code, daily volume, volume stats, tab state, and callbacks. Uses `ReferralContext.Provider` and hooks from `@veltodefi/hooks` and `@veltodefi/react-app`.
