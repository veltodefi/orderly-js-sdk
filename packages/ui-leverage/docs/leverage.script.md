...
Hook that wires account-level leverage UI to `useLeverage` from `@veltodefi/hooks`: current/max leverage, presets, slider state, and save/cancel handlers with toast feedback.
...
```typescript
import { useLeverageScript } from "@veltodefi/ui-leverage";
