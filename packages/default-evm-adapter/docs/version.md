...
- In a browser environment (`typeof window !== "undefined"`), sets `window.__ORDERLY_VERSION__["@veltodefi/default-evm-adapter"]` to the same version string, creating `__ORDERLY_VERSION__` if it does not exist.
- Does not mutate `window` in Node or other non-browser environments.

## Usage Example

```ts
import version from "@veltodefi/default-evm-adapter/version";
