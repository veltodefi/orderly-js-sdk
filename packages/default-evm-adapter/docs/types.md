...
| `contractManager`| `IContract`         | Yes      | Contract manager from `@veltodefi/core` for verification contract address. |
...
```ts
import type { EVMAdapterOptions, getWalletAdapterFunc } from "@veltodefi/default-evm-adapter";
