...
Presentational layout component that wraps `Scaffold` from `@veltodefi/ui-scaffold` with Trading Rewards–specific classNames and styling. Renders children inside the scaffold; left sidebar is passed as `null` (sidebar content is typically provided by the host app).
