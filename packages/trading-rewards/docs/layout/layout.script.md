...
Hook and enum for the Trading Rewards layout: sidebar navigation items (Trading / Affiliate), current path, and responsive hide behavior. Integrates with `@veltodefi/ui-scaffold` sidebar contract.
...
`SideBarProps` from `@veltodefi/ui-scaffold` plus:
