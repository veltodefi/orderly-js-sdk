...
HTTP fetcher used by SWR-based hooks. Wraps `@veltodefi/net` `get` with optional response formatter. Also exports `useQueryOptions` type and `noCacheConfig` for SWR.
...
```ts
import { fetcher, noCacheConfig, type useQueryOptions } from "@veltodefi/hooks";
```
