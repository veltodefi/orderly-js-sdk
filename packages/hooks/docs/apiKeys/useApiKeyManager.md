...
```tsx
import { useApiKeyManager, ScopeType } from "@veltodefi/hooks";
...
function ApiKeySettings() {
  ...
  const handleCreate = async () => {
    const { key, secretKey } = await generateOrderlyKey(
      ScopeType.tradeAndTrading,
    );
    ...
    <div>
      {keys?.map((k) => (
        <div key={k.orderly_key}>
          {k.orderly_key}
          <button onClick={() => removeOrderlyKey(k.orderly_key)}>
            Remove
          </button>
        </div>
      ))}
